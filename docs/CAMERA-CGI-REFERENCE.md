...
| 2026-09-09 | **Caché de probes: TTL 1 h + invalidación por eventos (D29/D31)**: la TTL de la caché de probes de `camera/adapters/yi-hack.js` pasa de 30 s a 1 h (`CAMERA_STATUS_CACHE_TTL_MS`, default; `0` desactiva): ahora es una red de seguridad, no el mecanismo de frescura — la frescura la garantizan las invalidaciones por eventos. Dos puntos de invalidación nuevos: (1) **transición MQTT offline→online** de la cámara (nuevo evento `camera-online` en el bus `mqttEvents`, emitido SOLO en reboot o recuperación de energía; NO en el primer online tras el arranque del NVR). Es el punto de invalidación correcto porque `FTP_UPLOAD` requiere reboot para aplicarse en la cámara (§12.6): la transición offline→online es exactamente el momento en que la config nueva entra en vigor, y además cubre reboots manuales y ediciones externas de la SD sin depender de topics no documentados. (2) **clip indexado por FTP** (`sendClipNotification` en `ftp.js`, solo yi-hack): auto-sanación — si un clip llega por FTP, cualquier probe en caché que diga lo contrario está obsoleto (además refresca lecturas de almacenamiento como `free_sd`). Consecuencia directa: **eliminación del espejo** `push/ftp-upload-state.js` — la decisión «¿la cámara sube clips por FTP?» en el trigger de movimiento de Web Push ya no lee un estado paralelo en memoria, sino el `system.conf` de la caché de probes del adapter (`getSystemConfig`, single-flight): `FTP_UPLOAD=yes` → silencio (la notificación «Nuevo clip» la envía `ftp.js` al indexar); `'no'`, campo ausente o probe fallido → notificación inmediata con la URL de la página de eventos de la cámara. Ver D31 en `docs/ARCHITECTURE.md`. |
